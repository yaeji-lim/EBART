...
# # GBEX Model 2 Parameter Functions
get_df <- function(X) {
...
for (p in p_values) {
  ...
  for (m in 1:M) {
    ...
    top5_count_erf <- sum(det_erf[m, ]) # 추가: 상위 5개 중 참 신호 개수
    ...
    top5_count_gbex <- sum(det_gbex[m, ]) # 추가: 상위 5개 중 참 신호 개수
    ...
    top5_count_ebart <- sum(det_ebart[m, ]) # 추가: 상위 5개 중 참 신호 개수
