...
for (p in p_values) {
  ...
  for (m in 1:M) {
    ...
    tp_erf <- sum(norm_erf[1:5] > 0.1)
    top5_count_erf <- sum(det_erf[m, ])
    ...
    tp_gbex <- sum(norm_gbex[1:5] > 0.1)
    top5_count_gbex <- sum(det_gbex[m, ]) 
    ...
    tp_ebart <- sum(norm_ebart[1:5] > 0.1)
    top5_count_ebart <- sum(det_ebart[m, ])
...
mse_plot <- ggplot(mse_long, aes(x = Method, y = MSE, fill = Method)) +
  ...
  scale_y_log10() + # 로그 스케일 필수 (그렇지 않으면 GBEX/EBART의 분포가 납작해져 보이지 않음)
...
# Bind variable selection list into a dataframe
...
var_sel_table_format <- var_sel_df %>%
  ...
  mutate(
    Metric = case_when(
      ...
      Metric == "TP"   ~ "Avg. True Positives (TP)",
      Metric == "Top5" ~ "Avg. Signals in Top 5",
      TRUE ~ Metric
    )
  ) %>%
...
p_plot <- ggplot(df_plot, aes(x=Variable, y=Importance, fill=Type)) +
  ...
  labs(title=sprintf("[Model 1] Relative Variable Importance Scores (p=%d)", p),
